...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("🆉ᴇʙʀᴀ 🆁ᴏʙᴏᴛ AI Already Activated In This Chat")
            return
        await lel.edit(f"🆉ᴇʙʀᴀ 🆁ᴏʙᴏᴛ AI Actived by {message.from_user.mention()} for users in {message.chat.title}")

    elif status == "OFF" or status == "off" or status == "Off":
        lel = await edit_or_reply(message, "`Processing...`")
        Escobar = remove_chat(int(message.chat.id))
        if not Escobar:
            await lel.edit("🆉ᴇʙʀᴀ 🆁ᴏʙᴏᴛ AI Was Not Activated In This Chat")
            return
        await lel.edit(f"🆉ᴇʙʀᴀ 🆁ᴏʙᴏᴛ AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("zebra", "Aco")
        test = test.replace("zebra", "Aco")
        test = test.replace("My god is Sumit Yadav", "I'm a Christian")
        test = test.replace("16", "9")
        test = test.replace("Sumit Yadav is my BestFriend.", "I'm single.")
        test = test.replace("My BestFriend is SumitYadav", "I'm single.")
        test = test.replace("@World_FriendShip_Zone", "Emikobot.ai")
        test = test.replace("I live in @World_FriendShip_Zone .", "I live in India, California.")
        test = test.replace("I was created by Sumit Yadav", "I made myself")
        test = test.replace(
            "Hello there I am Zebra...nice to meet u",
            "Hi, my friend! Do you want me to tell you a joke?")
        test = test.replace("Sumit Yadav is my owner", "Have the control right.")
        test = test.replace(
            "Hi, My name is Zebra Nice to meet you.",
            "Hi, my friend, what can I do for you today?")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Aco", "Zebra")
        response = response.replace("aco", "Zebra")
        response = response.replace("Luna", "Zebra")
        response = response.replace("luna", "Zebra")
        response = response.replace("I'm a Christian", "My god is Sumit Yadav")
        response = response.replace("9", "16")
        response = response.replace("I'm married to my job.", "I'm married with Cute_Shezhadi")
        response = response.replace("I'm single.", "My wife is Cute_Shezhadi ")
        response = response.replace("Emikobot.ai", "@World_FriendShip_Zone")
        response = response.replace("I live in India, California.", "I live in @World_FriendShip_Zone.")
        response = response.replace("I made myself", "I was Created by SumitYadav")
        response = response.replace(
                "Hi, my friend! Do you want me to tell you a joke?",
                "Hello there I am Zebra...nice to meet u")
        response = response.replace("Have the control right.", "Sumit Yadav is my owner.")
        response = response.replace(
                "Hi, my friend, what can I do for you today?",
                "Hi, My name is Zebra Nice to meet you")
        ...
        response = response.replace("I'm single.", "My Wife is Cute_Shezhadi")
        response = response.replace("Emikobot.ai", "@World_FriendShip_Zone")
        response = response.replace("I live in San Francisco, California.", "I live in @World_FriendShip_Zone.")
        response = response.replace("I made myself", "I was Created by Sumit Yadav")
        response = response.replace(
                "Hi, my friend! Do you want me to tell you a joke?",
                "Hello there I am Zbera...nice to meet u")
        response = response.replace("Have the control right.", "Sumit Yadav is my owner.")
        response = response.replace(
                "Hi, my friend, what can I do for you today?",
                "Hi, My name is Zebra Nice to meet you")
